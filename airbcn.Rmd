...
plot(serie,main="Miles de pasajeros de lineas aereas internacionales en el aeropuerto del Prat",type="o",ylim=c(0,4100))
...
```{r}
##Transformación logaritmica para homogeneizar la varianza
lnserie=log(serie)
plot(lnserie)
```
...
##Descomposición en componentes básicas (tendencia+estacionalidad+proceso estacionario)
plot(decompose(lnserie))
```

```{r}
##Análisis de los índices estacionales (plot mensual)
monthplot(lnserie)
```

```{r}
## Diferenciación estacional (eliminación de la componente estacional)
...
```{r}
## Nueva diferenciación regular (posible no estacionariedad)
...
(mod=arima(lnserie1,order=c(0,1,1),seasonal=list(order=c(2,1,0),period=12)))
...
##Detección de atípicos: hay una diferenciación regular y una de orden 12, fijamos el criterio a 2.8 y buscamos también LS
...
##Comparación serie observada con la serie linealizada (sin atípicos)
...
```{r}
##Efecto de los atípicos en la serie de logaritmos
plot(lnserie-lnserie.lin)
```

```{r}
##Identificación del modelo para la serie linealizada
...
```{r}
##Estimación del modelo para la serie linealizada
